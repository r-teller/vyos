...
class VyOsAPIClient:
    ...
    def set_config_value(self, config_path: Tuple[str], value: str):
        return self._vyos_post(
            path="/configure",
            data_dict={"op": "set", "path": config_path, "value": value},
        )
    ...
    def del_config_value(self, config_path: List[str]):
        return self._vyos_post(
            path="/configure", data_dict={"op": "delete", "path": config_path}
        )
        ...
        if resp.status_code < 200 or resp.status_code > 299:
            ...
            try:
                ...
                raise VyOSApiException(
                    ...
                    error=f"Api call returned non-successful status code: {resp.status_code}",
                )
            raise VyOSApiException(
                status_code=resp.status_code,
    ...
    def patch_config(
        self, config_text: str, patch_key: List[str], patch_value: str
    ) -> str:
    ...
    def load_configuration_from_file(self, file_path: str) -> dict:
        ...
        return self._vyos_post(
            path=f"/config-file", data_dict={"op": "load", "file": file_path}
        )

    def get_config(self, path: List[str] = []) -> dict:
        l.debug(f"Retrieving vyos current configuration")
        return self._vyos_post(
            path=f"/retrieve", data_dict={"op": "showConfig", "path": path}
        )
...
def _setup_client(
    ...
    if res.returncode != 0:
        raise RuntimeError(
            f"Client setup failed. Process output:\n {res.stdout}. \n\nProcess err:\n {res.stderr}."
        )
